...
class GPUARModelRunner(
    ...
    def sample_tokens(
        ...
        if self.use_async_scheduling:
            pp = get_pp_group()
            # Send the sampled token ids to the other PP ranks, which receive
            # them in the execute_model_state-less branch above. With
            # broadcast_pp_output the logits already reached every rank at
            # logits computation, so the extra send is redundant.
        ...
        use_async_omni_output = self._should_use_async_omni_output() and not accumulate_needed
        ...
        # Connector drain can TP-broadcast. Do it on the decode thread, not in
        # the deferred Omni payload builder.
